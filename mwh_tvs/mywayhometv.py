...
def to_dtype(x):
    if x is bool or x is int or x is float:
        try:
            x_dtype = np.dtype(x)
            return x_dtype.name
        except:
...
def stacked_space(space, k):
    new_dtype = np.dtype(to_dtype(space.dtype))
...
class TVEnv(gym.Wrapper):
    ...
    def __init__(self, env, mode, coverage=(1.0, 1.0), partitions=(8, 8), num_colors=16, allow_tv_off=True, 
                 color_format='rgb', rnd_batch=10000):
        gym.Wrapper.__init__(self, env)
        ...
        num_tv_actions = {'broken': 0, 'random': 1, 'madr': 4}[mode] + int(allow_tv_off)
        ...
        elif mode == 'random' and not allow_tv_off:
            self._operation = 'random_on'
        elif mode == 'random' and allow_tv_off:
            self._operation = 'random_off'
        elif mode == 'madr' and not allow_tv_off:
            self._operation = 'fixed_on'
        elif mode == 'madr' and allow_tv_off:
            self._operation = 'fixed_off'
    ...
    def reset(self):
        obs = self.env.reset()
        if self._operation in {'broken_off', 'random_off', 'fixed_off'}:
            self._tv_state = True
            self._tvon = 0
        if self._operation in {'fixed_on', 'fixed_off'}:
            self._totdist = 0
            self._maxdist = -1
            self._record = dict()
    ...
    def step(self, action):
        ...
        if self._operation == 'broken_on':
        ...
        elif self._operation == 'random_on':
            if action >= self._env_actions:
                ...
                for slc, color in zip(self._station_slices, self._tv_buffer.pop()):
                    self._last_tv[slc] = self._palette[color]
                obs[self._tv_slice] = self._last_tv
        elif self._operation == 'random_off':
        ...
        elif self._operation == 'fixed_off':
            tv_action = 0 if action < self._env_actions else (action - self._env_actions + 1)
            if tv_action == 1:
                self._tv_state = not self._tv_state
            info['step_tvon'] = self._tv_state
...
class MyWayHomeTVEnv(EnvMaker):
    ...
    def _make(self):
        ...
        if self._tv is not None:
            env = TVEnv(env, self._tv, self._tv_coverage, self._tv_partitions, self._tv_colors, self._allow_tv_off, self._color_format)
        if self._no_reward:
            env = NoRewardEnv(env)
    ...
    def _check_inputs(self):
        raise_error(self._name, 'tv', (is_none(self._tv), is_inset(self._tv, ('broken', 'random', 'madr'))))
        ...
        raise_error(self._name, 'tv_colors', (is_integer(self._tv_colors, 'pos'),))
